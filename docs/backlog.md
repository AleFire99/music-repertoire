...
- [x] In-app PDF viewer (native browser embed, no new frontend dependency) + server-rendered card-view preview thumbnails (via Poppler's `pdftoppm`) — issue #130

Investigated live against 30 real files from the user's own sheet-music folder (not assumed): the PDF text-layer heuristic returns nothing for 43% of real files, returns outright garbage for another 20% (a real duplicate-glyph rendering bug in several notation programs' PDF export), and its composer guess is actively misleading even when it "succeeds" (grabs subtitle/arrangement text, not composer names). Meanwhile the user's own filenames (`Composer - Title.pdf`, a convention they maintain by hand on every download specifically to work around this) were accurate on 28/30 files and are currently barely used. Confirmed fix direction: make filename parsing the primary signal, demote the (bug-fixed) text heuristic to a fallback, keep MusicBrainz as last-resort composer backfill — see issue below. Bulk/multi-file upload remains deferred until after this ships.
...
- **Content**: structured/templated Markdown pages with YAML front matter (not freeform prose) — a defined schema (title, composer, form, common keys, style/era, notable recordings, tags) plus a freeform body per page.
- **Format choice reasoning**: plain Markdown files, git-tracked, not database rows — directly consumable by a future RAG/agent pipeline with zero export step, regardless of which rendering tool sits on top.
- **Tooling**: MkDocs + Material theme, not Zensical (the same team's newer tool) — Zensical uses an identical Markdown+front-matter model and is `mkdocs.yml`-compatible, but is pre-1.0 and too immature for a wiki meant to last; migrating later is config-compatible, not a rewrite.
